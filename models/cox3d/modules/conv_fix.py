...
from models.co3d.modules.utils import unsqueezed
...
def convert_conv3d(
    instance: Conv3d, window_size: int = None, temporal_fill: FillMode = "replicate",
):
    if (
        instance.kernel_size[0] == 1
        and instance.padding[0] == 0
        and instance.stride[0] == 1
        and instance.dilation[0] == 1
    ):
        return unsqueezed(instance)
    else:
        return FixedConvCo3d.from_3d(instance, temporal_fill)


class FixedConvCo3d(_ConvNd):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        Attributes:
            ...
            bias (Tensor):   the learnable bias of the module of shape (out_channels). If :attr:`bias` is ``True``,
                            ...
                            :math:`k = \frac{groups}{C_\text{in} * \prod_{i=0}^{2}\text{kernel\_size}[i]}`
            state (Tensor):  a running buffer of partial computations from previous frames which are used for
        ...
        super(FixedConvCo3d, self).__init__(
            in_channels,
            out_channels,
            kernel_size,
            stride,
        ...
        self._reversed_padding_repeated_twice = (
            *_reverse_repeat_tuple((self.kernel_size[0] - 1, *self.padding[1:]), 2)[:5],
    ...
    def init_state(self, first_output: Tensor,) -> State:
        ...
        state_buffer = padding.repeat(
            ...
            1,
            1,
            1,  # TODO: <<--- Here the buffer size should differ or it's indexing?
            1,
            1,
        )
        state_index = 0
    ...
    def _forward(self, input: Tensor, prev_state: State) -> Tuple[Tensor, State]:
        assert (
            len(input.shape) == 5
        ), "A single frame with padded convolutions along shape[2] should be given."

        # Pad input along spatial dimensions
        x = F.pad(
            input,
            (*self._reversed_padding_repeated_twice[:4], 0, 0),
            mode=("constant" if self.padding_mode == "zeros" else self.padding_mode),
        )
        # Pad along temporal dims (must be zeros)
        x = F.pad(
            x,
            (0, 0, 0, 0, *self._reversed_padding_repeated_twice[4:]),
            mode="constant",
        )

        # Propagate through zero-padded conv
        x = F.conv3d(
            input=x,
            weight=self.weight,
            bias=None,
        ...
        x_out, x_rest = x[:, :, : self.padding[0] + 1].clone(), x[:, :, 1:]

        # Prepare previous state
        buffer, index = prev_state or self.init_state(x_rest)

        tot = len(buffer)
        for j in range(self.padding[0] + 1):
            for i in range(tot - j):
                # TODO: add support for other padding modes
                x_out[:, :, j] += buffer[(i + j + index) % tot, j, :, :, tot - i - 1]

            if self.bias is not None:
                x_out[:, :, j] += self.bias[None, :, None, None]
    ...
    def forward3d(self, input: Tensor):
        ...
        pad_start = [
            self.make_padding(input[:, :, 0]).unsqueeze(2)
            for _ in range(self.padding[0])
        ]
        inputs = [input[:, :, t].unsqueeze(2) for t in range(T)]
        pad_end = [
            self.make_padding(input[:, :, -1]).unsqueeze(2)
            for _ in range(self.padding[0])
        ]
