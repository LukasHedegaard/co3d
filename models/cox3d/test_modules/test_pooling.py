...
def test_avg_pool():
    ...
    output = AvgPoolCo3d(window_size=2, kernel_size=(2, 2)).forward3d(example_clip)
...
def test_global_avg_pool():
    ...
    output = rpool.forward3d(example_clip)
    assert torch.allclose(output, target)

    # Now that memory is full (via `forward3d`), pooling works as expected for subsequent frames
...
def test_max_pool():
    ...
    output = MaxPoolCo3d(window_size=2, kernel_size=(2, 2)).forward3d(example_clip)
    sub_output = torch.stack(
        [
            output[:, :, 0],
            output[:, :, 2],
...
def test_global_max_pool():
    ...
    output = AdaptiveMaxPoolCo3d(window_size=4, output_size=(1, 1)).forward3d(
        example_clip
    )
    assert torch.allclose(output, target)
